...
export class AdminService {
  ...
  async getDashboard(){
    ...
    const soldByProduct=new Map<string,number>(),sold30ByProduct=new Map<string,number>();
    for(const order of completed) for(const item of order.items||[]){soldByProduct.set(item.productId,(soldByProduct.get(item.productId)||0)+item.quantity);if(new Date(order.createdAt)>=thirtyDaysAgo)sold30ByProduct.set(item.productId,(sold30ByProduct.get(item.productId)||0)+item.quantity)}
    ...
    return{todaysSales:sum(completed.filter(o=>new Date(o.createdAt)>=day)),monthlyRevenue:sum(completed.filter(o=>new Date(o.createdAt)>=month)),ordersWaiting:orders.filter(o=>![OrderStatus.Delivered,OrderStatus.Cancelled,OrderStatus.Returned,OrderStatus.Refunded].includes(o.status)).length,lowStockProducts:products.filter(p=>p.isActive&&stockFor(p)<=5).length,returningCustomers:customers,totalProducts:products.length,activeProducts:products.filter(p=>p.isActive).length,deliveries,returns,openTickets:records.filter(r=>r.resource==='support-tickets'&&r.data?.status!=='Closed').length,pendingReviews:records.filter(r=>r.resource==='reviews'&&r.data?.status==='Pending').length,openQuestions:records.filter(r=>r.resource==='product-questions'&&r.data?.status!=='Answered').length,recentOrders:orders.slice(0,5).map(o=>({id:o.id,orderNumber:o.orderNumber,status:o.status,total:Number(o.total),createdAt:o.createdAt})),lowStock:products.filter(p=>p.isActive&&stockFor(p)<=5).slice(0,5).map(p=>({id:p.id,name:p.name,sku:p.sku,stock:stockFor(p)})),inventoryAnalysis:{unitsSold,unitsPurchased,unitsRemaining,totalSalesRevenue:sum(completed),inventoryRetailValue,netStockFlow:unitsPurchased-unitsSold},stockForecast:forecast}
  }
}
